...
- 有判断力但需要激发的编剧、制片人、策划——你知道什么是好的，但面对空白页需要有人先扔几个 75 分的选项出来。
- 希望能将过往的创作经验逐步地、合理地整合进AI工作流的创作者。
...
- **index.html（页面版）：** 直接把提示词给你信赖的网页版大模型，或者调用 API，没有平台 harness 干扰，方法论直接跟模型对话。这是创意输出质量最可控的版本。
