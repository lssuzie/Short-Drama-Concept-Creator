...
async function loadCloudData(){
  ...
  // 加载历史
  var histResult=await sb.from('history').select('*').eq('user_id',uid).order('ts',{ascending:false}).limit(50);
  if(histResult.data){
    _memHistory=histResult.data.map(function(r){
      return{ts:r.ts,aud:r.aud,genre:r.genre,tags:r.tags||[],prompt:r.prompt,model:r.model};
    });
  }
}
...
function clearHist(){
  ...
  // 同步清空 Supabase
  if(sb&&sbCurrentUser()){
    sb.from('history').delete().eq('user_id',sbCurrentUser().id).catch(function(){});
  }
}
...
async function testSupabase(){
  if(!sb){console.log('Supabase client not initialized');return}
  var user=sbCurrentUser();
  console.log('Current user:',user);
  if(!user){console.log('Not logged in');return}
  try{
    var r=await sb.from('history').select('*').eq('user_id',user.id).limit(1);
    console.log('History query result:',r);
    alert('Supabase 连接正常！history 表有 '+((r.data&&r.data.length)||0)+' 条数据');
  }catch(e){
    console.error('Supabase test error:',e);
    alert('Supabase 出错: '+e.message);
  }
}
